...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        // 1. 스토리보드: 컨트롤러 및 관련된 뷰를 정의할 때에는 뷰컨트롤러 클래스를 직접 초기화할 수 없기 때문에 instantiateViewController(withIdentifier:) 를 통해 프로그래밍 방식으로 인스턴스화된다.
...
//        let vc = SettingViewController(nibName: "SettingViewController", bundle: nil)
        ...
        window?.makeKeyAndVisible() // iOS13, 지정한 rootViewController가 상호작용을 받는 현재 화면으로 세팅
    }
